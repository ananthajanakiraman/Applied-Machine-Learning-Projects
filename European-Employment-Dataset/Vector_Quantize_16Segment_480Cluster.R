...
create_big_data_from_csv_dir <- function(hmp, directory, ids) {
  ...
  for (i in 1:length(data.list)) {
    ...
    df2 <<- split(df1,(as.numeric(rownames(df1))-1) %/% 16)
    
    if (length(df2[[length(df2)]][[1]]) < 16) {
      df2 <<- df2[1:(length(df2)-1)]
    }
    
    for (j in 1:length(df2)) {
      colnames(df2[[j]]) <<- c("x","y","z")
      if(length(grep("/Test/",directory,ignore.case = TRUE))>0) {
        final_test_df <<- rbind.data.frame(final_test_df,cbind.data.frame(t(df2[[j]]$x),t(df2[[j]]$y),t(df2[[j]]$z),hmp,i))
      } else {
        final_train_df <<- rbind.data.frame(final_train_df,cbind.data.frame(t(df2[[j]]$x),t(df2[[j]]$y),t(df2[[j]]$z),hmp,i))
      }
...
colnames(final_train_df) <- c(1:48,"hmp","signal")

dictionary_df <- final_train_df
training_df <- final_train_df
training_df <- cbind(training_df,clusterno=0)
k <- 480
km <- kmeans(final_train_df[,1:48],k,iter.max = 30)

center_matrix <- km$centers

for(i in 1:nrow(training_df)) {
  
  center_matrix_with_train_vect <- rbind(center_matrix,vect=training_df[i,1:48])
...
colnames(final_test_df) <- c(1:48,"hmp","signal")
...
for(i in 1:nrow(testing_df)) {
  ...
  center_matrix_with_test_vect <- rbind(center_matrix,vect=testing_df[i,1:48])
