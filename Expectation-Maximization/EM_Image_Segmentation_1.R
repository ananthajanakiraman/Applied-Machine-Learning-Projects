...
input_image <- readJPEG("RobertMixed03.jpg")
...
r1 <- matrix(data=r,ncol=640,byrow = TRUE)
g1 <- matrix(data=g,ncol=640,byrow = TRUE)
b1 <- matrix(data=b,ncol=640,byrow = TRUE)
...
initial_pi <- cbind(initial_pi,initial_pi[,1]/307200)
initial_mu <- initial_cluster$centers

mu_matrix <- initial_mu
pi_matrix <- as.matrix(initial_pi[,2])
work_matrix <- matrix(data=1,nrow = 307200,ncol=k,byrow = TRUE)
log_w_ij <- matrix(data=0,nrow = 307200,ncol=k,byrow = TRUE)
hold_mat <- matrix(data=0,nrow = 307200,ncol=1,byrow = TRUE)
iteration=0
...
#LOW PERFORMANCE CODE!! - 5 Minutes for loop completion in each iteration
#for(i in 1:307200) {
#  for (j in 1:k) {
...
#LOW PERFORMANCE CODE!! - 2Mins 50secs for loop completion in each iteration
#for(j in 1:k) {
#  rep_matrix <- matrix(data=mu_matrix[j,],nrow=307200,ncol=3,byrow = TRUE)
#  temp_matrix <- xi_image[,c(1:3)] - rep_matrix
#  for(i in 1:307200) { hold_mat[i,] <- -(tcrossprod(temp_matrix[i,],t(temp_matrix[i,])))/2+log(pi_matrix[j,]) }
#  if (j==1) { work_matrix <- cbind(hold_mat)} else { work_matrix <- cbind(work_matrix,hold_mat)}
#}

#BEST PERFORMANCE - Took 5 secs for completion in each iteration
for(j in 1:k) {
  rep_matrix <- matrix(data=mu_matrix[j,],nrow=307200,ncol=3,byrow = TRUE)
  temp_matrix <- xi_image[,c(1:3)] - rep_matrix
...
pi_matrix_maximized <- pi_matrix_maximized/307200
...
r1 <- matrix(data=xi_image_1[,1],ncol=640,byrow = TRUE)
g1 <- matrix(data=xi_image_1[,2],ncol=640,byrow = TRUE)
b1 <- matrix(data=xi_image_1[,3],ncol=640,byrow = TRUE)
col <- rgb(r1,g1,b1,maxColorValue = 255)
dim(col) <- dim(r1)
grid.raster(col,interpolate = FALSE)
